...
export const budgetApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getBudgets: builder.query<ApiResponse<BudgetWithProgress[]>, void>({
      query: () => '/api/budgets',
      providesTags: ['Budget'],
    }),

    getBudget: builder.query<ApiResponse<BudgetWithProgress>, string>({
      query: (id) => `/api/budgets/${id}`,
      providesTags: (_result, _error, id) => [{ type: 'Budget', id }],
    }),

    getBudgetAlerts: builder.query<ApiResponse<BudgetAlert[]>, void>({
      query: () => '/api/budgets/alerts',
      providesTags: ['Budget'],
    }),

    createBudget: builder.mutation<
      ApiResponse<BudgetWithProgress>,
      CreateBudgetRequest
    >({
      query: (body) => ({
        url: '/api/budgets',
